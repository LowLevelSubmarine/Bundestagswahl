...
class TodayText(val todayData: DataPoint, val lastData: DataPoint, val dawum: Dawum) {
    ...
    inner class PartyChange(val from: Float?, val to: Float?, val partyId: Long) {
        ...
        val change: Float? = if (from != null && to != null) from - to else null

        @Override
        override fun toString(): String {
            val partyName = this@TodayText.dawum.parties[partyId]!!.name
            if (this.change != null) return partyName + "%.2f".format(change)
